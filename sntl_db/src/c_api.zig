...
pub const AppendLog = struct {
    ...
    pub fn append(self: *AppendLog, data: []const u8) !u64 {
        ...
        const checksum = @as(u64, std.hash.Crc32.hash(data));
...
pub export fn sntl_db_flag_high_value_target(ip_ptr: [*]const u8, ip_len: usize) callconv(.c) bool {
    global_mutex.lock();
    defer global_mutex.unlock();

    if (ip_len == 0) return false;
    const ip = ip_ptr[0..ip_len];

    if (global_log) |*log| {
        var record_buf: [256]u8 = undefined;
        const prefix = "APT_HIGH_VALUE_TARGET|IP=";
        if (prefix.len + ip.len >= record_buf.len) return false;

        @memcpy(record_buf[0..prefix.len], prefix);
        @memcpy(record_buf[prefix.len..][0..ip.len], ip);
        const total_len = prefix.len + ip.len;

        _ = log.append(record_buf[0..total_len]) catch return false;
        return true;
    }
    return false;
}
